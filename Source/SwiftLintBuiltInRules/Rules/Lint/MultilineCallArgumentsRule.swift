...
struct MultilineCallArgumentsRule: Rule {
    ...
    enum Reason {
        static let singleLineMultipleArgumentsNotAllowed = "Single-line calls with multiple arguments are not allowed"

        static func tooManyArgumentsOnSingleLine(max: Int) -> String {
            "Too many arguments on a single line (max: \(max))"
        }

        static let eachArgumentMustStartOnOwnLine = "In multi-line calls, each argument must start on its own line"
...
private extension MultilineCallArgumentsRule {
    final class Visitor: ViolationsSyntaxVisitor<ConfigurationType> {
        ...
        private func reasonedViolations(
            ...
            return duplicateArgumentStartLineViolations(arguments: arguments, callNode: callNode)
                + newlineAfterCommaViolations(arguments: arguments, callNode: callNode)
        }
                ...
                if !seen.insert(argumentLine).inserted {
                    ...
                    let correctionStart =
                        if let comma = prevArgument.trailingComma, comma.presence != .missing {
                           comma.endPositionBeforeTrailingTrivia
                        } else {
                           prevArgument.endPositionBeforeTrailingTrivia
                        }
