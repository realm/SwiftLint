...
private extension FileHeaderRule {
    final class Visitor: ViolationsSyntaxVisitor<ConfigurationType> {
        ...
        private func processTrivia(_ trivia: Trivia,
                                   ...
                                   lastEnd: inout AbsolutePosition?) -> (foundNonComment: Bool, void: Void) {
            for piece in trivia {
                ...
                if piece.isComment && !piece.isDocComment {
                    ...
                    return (foundNonComment: true, void: ())
                }
            }
            return (foundNonComment: false, void: ())
        }
        ...
        private func checkForbiddenPattern(in headerContent: String, startingAt headerStart: AbsolutePosition) {
            guard let forbiddenRegex = configuration.forbiddenRegex(for: file) else { return }

            let contentRange = NSRange(location: 0, length: headerContent.utf16.count)
            guard let firstMatch = forbiddenRegex.firstMatch(in: headerContent, options: [], range: contentRange) else {
                return
            }
        ...
        private func checkRequiredPattern(_ requiredRegex: NSRegularExpression?,
            ...
            guard let requiredRegex else { return }

            let contentRange = NSRange(location: 0, length: headerContent.utf16.count)
            if requiredRegex.firstMatch(in: headerContent, options: [], range: contentRange) == nil {
                violations.append(ReasonedRuleViolation(
