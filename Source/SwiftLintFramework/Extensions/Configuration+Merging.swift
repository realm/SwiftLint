...
extension Configuration {
    ...
    private func mergeCustomRules(mergedRules: [Rule], configuration: Configuration) -> [Rule] {
        ...
        case let .allowlisted(allowlistedRules):
            customRulesFilter = { allowlistedRules.contains($0.identifier) }
    ...
    private func mergingRules(with configuration: Configuration) -> [Rule] {
        ...
        case .allowlisted(let allowlistedRules):
            ...
            regularMergedRules = Set(configuration.rules.map(HashableRule.init))
                ...
                .map { $0.rule }
                .filter { rule in
                    return allowlistedRules.contains(type(of: rule).description.identifier)
                }
