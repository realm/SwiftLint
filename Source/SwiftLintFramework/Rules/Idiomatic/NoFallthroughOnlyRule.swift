...
public struct NoFallthroughOnlyRule: ASTRule, ConfigurationProviderRule, AutomaticTestableRule {
    ...
    public func validate(file: SwiftLintFile,
        ...
        guard kind == .case,
            ...
            case let nsstring = file.linesContainer,
            let range = nsstring.byteRangeToNSRange(start: offset, length: length),
            let colonLocation = findCaseColon(text: file.linesContainer.nsString, range: range)
        else {
            return []
        }
        ...
        if nsstring.nsString.substring(with: nsRange) == "fallthrough" && nonCommentCaseBody[0].1 == [.keyword] &&
