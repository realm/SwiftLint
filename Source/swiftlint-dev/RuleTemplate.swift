...
extension SwiftLintDev {
    struct RuleTemplate: AsyncParsableCommand {
        ...
        func run() async throws {
            let ruleDirectory = FileManager.default.currentDirectoryPath
                .appending("/Source")
                .appending("/SwiftLintBuiltInRules")
                .appending("/Rules")
            let ruleLocation = ruleDirectory.appending("/" + kind.rawValue.capitalized)
            ...
            let rulePath = ruleLocation.appending("/\(name)Rule.swift")
            ...
            let configPath = ruleDirectory
                .appending("/RuleConfigurations")
                .appending("/\(name)Configuration.swift")
            ...
            let testPath = FileManager.default.currentDirectoryPath
                .appending("/Tests")
                .appending("/BuiltInRulesTests")
                .appending("/\(name)RuleTests.swift")
...
private extension SwiftLintDev.RuleTemplate {
    ...
    var testTemplate: String {
        ...
        final class \(name)RuleTests: SwiftLintTestCase {
            func test() {
                verifyRule(MyRuleRule.description, ruleConfiguration: [])
            }
        }
...
extension RuleKind: ExpressibleByArgument {
    public init?(argument: String) {
        if let new = Self(rawValue: argument) {
            self = new
        }
        return nil
    }
