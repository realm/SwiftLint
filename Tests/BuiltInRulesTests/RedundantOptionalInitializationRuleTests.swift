...
final class RedundantOptionalInitializationRuleTests: SwiftLintTestCase {
    func testDefaultConfiguration() {
        let config = RedundantOptionalInitializationConfiguration()
    ...
    func testApplyingCustomConfiguration() throws {
        var config = RedundantOptionalInitializationConfiguration()
        try config.apply(
            configuration: [
                "severity": "error",
        ...
        XCTAssertEqual(
            config.excludedAttributeNames,
            ["Parameter", "MyCustomAttribute"]
        )
    }
    ...
    func testInvalidKeyInCustomConfiguration() async throws {
        ...
        XCTAssertEqual(
            console,
            """
            warning: Configuration for 'redundant_optional_initialization' rule contains \
            the invalid key(s) 'invalidKey'.
            """
        )
    }
