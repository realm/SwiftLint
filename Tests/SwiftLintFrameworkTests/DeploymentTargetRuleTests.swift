...
final class DeploymentTargetRuleTests: SwiftLintTestCase {
    func testMacOSAttributeReason() async {
        let example = Example("@available(macOS 10.11, *)\nclass A {}")
        let violations = await self.violations(example, config: ["macOS_deployment_target": "10.14.0"])
    ...
    func testWatchOSConditionReason() async {
        let example = Example("if #available(watchOS 4, *) {}")
        let violations = await self.violations(example, config: ["watchOS_deployment_target": "5.0.1"])
    ...
    func testiOSNegativeAttributeReason() async throws {
        ...
        let violations = await self.violations(example, config: ["iOS_deployment_target": "15.0"])
    ...
    private func violations(_ example: Example, config: Any?) async -> [StyleViolation] {
