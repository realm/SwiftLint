...
class DisableAllTests: XCTestCase {
    ...
    func testViolatingPhrase() {
        for violatingPhrase in violatingPhrases {
            XCTAssertEqual(
                TestHelpers.violations(violatingPhrase.with(code: violatingPhrase.code + "\n")).count,
                1,
                #function,
    ...
    func testDisableAll() {
        for violatingPhrase in violatingPhrases {
            ...
            XCTAssertEqual(
                TestHelpers.violations(protectedPhrase).count,
                0,
                #function,
                file: violatingPhrase.file,
                line: violatingPhrase.line)
    ...
    func testEnableAll() {
        for violatingPhrase in violatingPhrases {
            let unprotectedPhrase = violatingPhrase.with(code: """
                ...
                """)
            XCTAssertEqual(
                TestHelpers.violations(unprotectedPhrase).count,
                1,
                #function,
                file: violatingPhrase.file,
                line: violatingPhrase.line)
