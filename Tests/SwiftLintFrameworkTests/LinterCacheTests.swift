...
class LinterCacheTests: XCTestCase {
    ...
    func testUnchangedFilesReusesCache() {
        let helper = makeCacheTestHelper(dict: ["allowlist_rules": ["mock"]])
        let file = "foo.swift"
    ...
    func testConfigFileReorderedReusesCache() {
        let helper = makeCacheTestHelper(dict: ["allowlist_rules": ["mock"], "disabled_rules": []])
        let file = "foo.swift"
        let violations = helper.makeViolations(file: file)

        cacheAndValidate(violations: violations, forFile: file, configuration: helper.configuration)
        let configuration2 = helper.makeConfig(dict: ["disabled_rules": [], "allowlist_rules": ["mock"]])
    ...
    func testConfigFileWhitespaceAndCommentsChangedOrAddedOrRemovedReusesCache() throws {
        let helper = makeCacheTestHelper(dict: try YamlParser.parse("allowlist_rules:\n  - mock"))
        let file = "foo.swift"
        let violations = helper.makeViolations(file: file)
    ...
    func testConfigFileUnrelatedKeysChangedOrAddedOrRemovedReusesCache() {
        let helper = makeCacheTestHelper(dict: ["allowlist_rules": ["mock"], "reporter": "json"])
        let file = "foo.swift"
        let violations = helper.makeViolations(file: file)

        cacheAndValidate(violations: violations, forFile: file, configuration: helper.configuration)
        let configuration2 = helper.makeConfig(dict: ["allowlist_rules": ["mock"], "reporter": "xcode"])
        XCTAssertEqual(cache.violations(forFile: file, configuration: configuration2)!, violations)
        let configuration3 = helper.makeConfig(dict: ["allowlist_rules": ["mock"]])
        XCTAssertEqual(cache.violations(forFile: file, configuration: configuration3)!, violations)
    }
    ...
    func testCustomRulesChangedOrAddedOrRemovedCausesAllFilesToBeReLinted() {
        let initialConfig = Configuration(
            dict: [
                "allowlist_rules": ["custom_rules", "rule1"],
        ...
        // Removal
        validateNewConfigDoesntHitCache(dict: ["allowlist_rules": ["custom_rules"]], initialConfig: initialConfig)
    }
    ...
    func testAllowlistRulesChangedOrAddedOrRemovedCausesAllFilesToBeReLinted() {
        let initialConfig = Configuration(dict: ["allowlist_rules": ["nesting"]])!
        cacheAndValidateNoViolationsTwoFiles(configuration: initialConfig)

        // Change
        validateNewConfigDoesntHitCache(dict: ["allowlist_rules": ["todo"]], initialConfig: initialConfig)
        // Addition
        validateNewConfigDoesntHitCache(dict: ["allowlist_rules": ["nesting", "todo"]], initialConfig: initialConfig)
        // Removal
        validateNewConfigDoesntHitCache(dict: ["allowlist_rules": []], initialConfig: initialConfig)
    }
