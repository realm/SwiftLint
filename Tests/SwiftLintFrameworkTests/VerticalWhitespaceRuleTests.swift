...
class VerticalWhitespaceRuleTests: XCTestCase {
    ...
    func testAttributesWithMaxEmptyLines() async throws {
        ...
        try await verifyRule(maxEmptyLinesDescription,
                             ruleConfiguration: ["max_empty_lines": 2])
    }

    func testAutoCorrectionWithMaxEmptyLines() async throws {
        let maxEmptyLinesDescription = VerticalWhitespaceRule.description
    ...
    func testViolationMessageWithMaxEmptyLines() async throws {
        guard let config = makeConfig(["max_empty_lines": 2], ruleID) else {
            XCTFail("Failed to create configuration")
            return
        }
        let allViolations = try await violations(Example("let aaaa = 0\n\n\n\nlet bbb = 2\n"), config: config)
    ...
    func testViolationMessageWithDefaultConfiguration() async throws {
        let allViolations = try await violations(Example("let aaaa = 0\n\n\n\nlet bbb = 2\n"))
